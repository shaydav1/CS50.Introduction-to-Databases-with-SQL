...
SELECT first_name, last_name, salary, HR, pe.year
FROM players p join salaries s on p.id=s.player_id
 join performances pe on p.id=pe.player_id and pe.year=s.year
ORDER BY p.id, pe.year desc, HR desc, salary desc;
